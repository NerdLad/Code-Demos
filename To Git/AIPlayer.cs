...
public class AIPlayer : MonoBehaviourPunCallbacks
{
    ...
    //AI delays
    public float aIDelay1;
    public float aIDelay2Min;
    public float aIDelay2Max;
    private bool addDelay2 = true;
    ...
    private void Update() {
        ...
        if (!isProcessingTurn&&!GameModel.GetInstance().isGameOver) {
    ...
    public void ProcessTurn() {
        ...
        foreach (Move baseMove in GameModel.GetInstance().PossibleMoves) {
            ...
            newEval = Minimax(copyCheckersData(GameModel.GetInstance().Board), baseMove, aIDepth, true, 2, -1000, 1000);
    ...
    //delay for right after the player moves
     IEnumerator DelayProcess() {
        float delayTime;

        if (addDelay2) {
           delayTime = aIDelay1 + (Random.Range(aIDelay2Min, aIDelay2Max));
        }
        else {
            delayTime = aIDelay1;
        }      
        
        print("Processing in: "+delayTime);
    ...
    public int Minimax(CheckerData[,] virtualBoard, Move move, int depth, bool maximizingPlayer, int player,
        ...
        virtualPossibleMoves = GameModel.GetPossibleMoves(virtualBoard, 8, NotPlayer(player),true);
        ...
        if (depth <= 0 || CheckVirtualVictory(copyCheckersData(virtualBoard)) || virtualPossibleMoves.Count == 0) {
            ...
            if (maximizingPlayer) {
                return Evaluate(virtualBoard, 2);
            }
            else {
                print("here");
                return Evaluate(virtualBoard, 1);
            }
        }
        ...
        if (maximizingPlayer) {
            ...
            foreach (Move possibleMove in virtualPossibleMoves) {
                float eval;
                eval = Minimax(copyCheckersData(virtualBoard), possibleMove, depth - 1, false, 1, alpha, beta);
        ...
        //same as above, but for the white player
            ...
            foreach (Move possibleMove in virtualPossibleMoves) {
                float eval;
                eval = Minimax(copyCheckersData(virtualBoard), possibleMove, depth - 1, true, 2, alpha, beta);
